...
def difficulty_settings(tipo, dificultad):
    ...
    if tipo == "Deslizante PRO (4x4)":
        # modo deslizante fijo 4x4 pero mezcla distinta
        if dificultad == "bajo":
...
def slide_click(r, c):
    ...
    # si la casilla es vecina al vacío, se mueve
...
def slide_solve():
    # resolver = poner en estado final (simple, porque solver real es largo)
    size = st.session_state.slide_size
...
def memo_new_game(dificultad):
    ...
    # emojis simples (sin fuentes raras)
    pool = ["🍎","🍌","🍇","🍉","🍓","🍒","🥝","🍍","🥥","🍑","🍋","🍊","🥭","🍐","🥕","🌽","🍪","🍩","🍫","🧁","⚽","🏀","🎲","🎯","🎹","🎸","🚗","✈️","🚀","🧩"]
    random.shuffle(pool)
    vals = pool[:pairs] * 2
    random.shuffle(vals)
...
def memo_click(i):
    ...
    if len(st.session_state.memo_open) == 2:
        ...
        if va == vb:
            ...
            # lock y cerrar en el próximo rerun con un pequeño delay visual
            st.session_state.memo_lock = True

def memo_post_check():
    # si lock activo, esperamos un poquito y cerramos
    if st.session_state.memo_lock:
        time.sleep(0.6)
        st.session_state.memo_open = []
        st.session_state.memo_lock = False
    ...
    with mid:
        ...
        for r in range(rows):
            ...
            for c in range(cols):
                ...
                row_cols[c].button(label, key=f"memo_{idx}", disabled=disabled, on_click=memo_click, args=(idx,))
                idx += 1
...
# aplicar selección (sin romper estado)
...
# Nuevo / Reiniciar / Resolver
if nuevo:
    # Nuevo = respeta selección actual pero crea juego desde cero
...
if reiniciar:
    # Reiniciar = mismo tipo/dificultad, vuelve a mezclar
    if st.session_state.tipo == "Deslizante":
        slide_new_game(st.session_state.dificultad, modo="Deslizante")
    elif st.session_state.tipo == "Deslizante PRO (4x4)":
        slide_new_game(st.session_state.dificultad, modo="Deslizante PRO (4x4)")
